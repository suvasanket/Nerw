...
extension String {
    ...
    func index(of aString: String, startingFrom position: Int? = 0) -> String.Index? {
        ...
        let range: Range<Index> = Range<Index>.init(
            uncheckedBounds: (lower: start, upper: self.endIndex))
    ...
    func lastIndexOf(_ searchStr: String, position: Int? = 0) -> String.Index? {
        ...
        let r: Range<Index> = Range<Index>.init(
            uncheckedBounds: (
                lower: self.startIndex,
                upper: self.index(self.startIndex, offsetBy: min(start + searchLen, len))
            ))
