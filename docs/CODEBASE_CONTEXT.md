...
- **Key Components**:
    ...
    - **`MathAndConversions/`**: Unified directory consolidating all calculation, unit, currency, timezone, date, and design conversion engines.
        - `MathConversionService.swift`: Main coordinator for `.mathConversion` queries. Orchestrates routing to specialized engines, clipboard copying on Enter, and building rich Peek preview cards.
        - `MathEngine.swift`: Mathematical evaluator with `JavaScriptCore`, advanced trig (`cot`, `sec`, `csc`, `acot`, `asec`, `acsc`, hyperbolics), degree modes, number suffixes (`1K`=1,000, `1.5M`=1,500,000), tip calculations (`15% tip on 42`), ratios/aspect scaling (`ratio of 3 to 5`, `scale 16:9 to width 1920`), percentage changes (`% increase from 50 to 75`), and base conversions (`hex`, `bin`, `oct`, `dec`).
        - `UnitConversionEngine.swift`: High-precision unit conversion across 13 physical/digital dimensions with compound units, human timespan formatting (`145 mins to timespan`), and work planning (`55h in workdays`, `workhours in 2026`).
        - `CurrencyConversionEngine.swift`: Multi-currency converter backed by Frankfurter API (ECB), offline baseline reference table, 12h disk caching, and shorthand token expansion (`USD1K`, `10K in EUR`).
        - `DateTimeEngine.swift`: Calendar and date calculations including date/time math (`August 5 + 5`, `3:45pm + 5`), relative natural dates (`monday in 3 weeks`, `next friday`), date countdowns & distance (`days until next sunday`, `days until november`, `days to nov`, `days in november`, `days since 1 Jan 2026`, `days until 31 Mar`, `days left in quarter/year/month`, `weeks until christmas`), holidays (`halloween`, `thanksgiving`, `christmas`), and ISO 8601 / Unix epoch conversions.
        - `WorldClockEngine.swift`: World clock and timezone converter with 500+ global cities, top 100 airport IATA codes (`JFK`, `LHR`, `DXB`, `HND`), cross-city conversions (`5pm ldn in sf`), timezone differentials (`time diff Paris`), and time projections (`time in 4 hours in SF`).
        - `DesignUnitEngine.swift`: UI and print design unit conversions supporting custom DPI/PPI (`2 inches in px at 72 ppi`, `1920px in cm at 300 dpi`) and CSS typography (`16px in rem`, `1.5rem in px`, `24px in pt`).
...
| File | Purpose |
|---|---|
| `NerwHubWindowController.swift` | Manages the native `NerwHubWindow` (`.titled`, `.closable`, `.miniaturizable`, `.resizable`, `.fullSizeContentView`) with `#1e1e1e` background, initial `920x580` size, unconstrained expansion, traffic lights padded from top and left, and window lifecycle. |
| `NerwHubViewController.swift` | Master split controller managing the Finder-style layout: left sidebar (`NerwHubSidebarView`), top navigation/search bar (`NerwHubTopBarView`), tab content container, bottom status bar (`NerwHubStatusBar`), keyboard routing (`⌘1-3`, `⌘F`, `⌘K`, `⌘S`, `⌘W`, `⌘Q`), sidebar toggle animation, command palette, and floating input editor. |
| `NerwHubSidebarView.swift` | Native macOS sidebar backed by `.sidebar` visual effect material, tab item rows ("Memory", "Bookmarks", "Conversations"), live badge counters, translucent selection pills (`NSColor.white.withAlphaComponent(0.12)`), and top-right toggle sidebar button (`sidebar.leading`). |
| `NerwHubTopBarView.swift` | Top bar featuring large bold tab titles (22pt bold), `LiquidDropletButton` (`command.circle` with smooth frosted glass hover highlight) positioned to the left of the search pill, collapsed sidebar toggle button (`sidebar.leading`), collapsible flex width on shrink, and live search filtering with Esc unfocusing. |
| `HubCommandPaletteViewController.swift` | Compact floating glass modal (340x240) with deep shadow, 12pt typography, 28pt search pill, 28pt command rows, and fast keyboard navigation for executing contextual actions (`Search`, `Toggle Sidebar`, `Switch to Tab`, `Edit`, `Delete`, `Open`, `Deselect`, `Refresh`, `Clear All Conversations`, `Close Window`) on the active tab and selection. |
...
| `Tabs/BaseHubListTab.swift` | Generic list controller base (`BaseHubListTab<Item>`) implementing `BaseHubTabProtocol`, real-time `filter(with:)` support, `onCountChanged` badge reporting, `selectedIndex`, `deselectAll()`, auto-scrolling, and row selection states. Content stretches when window expands and wraps/collapses without overflow when window shrinks. |
...
- **Sidebar Toggle (`Cmd+S`)**: Dedicated `LiquidDropletButton` with SF Symbol `sidebar.leading` in the top right of the sidebar collapses the sidebar with smooth animation. When collapsed, a toggle button appears in the top bar right after traffic lights. Pressing `Cmd+S` toggles the sidebar anytime.
- **Liquid Droplet Button**: Frosted glass circular button with smooth opacity and specular border transitions on hover and press (no pop-up scaling) for the Command Palette (`⌘K`) and Sidebar Toggle (`⌘S`).
...
- **Keyboard Navigation & Deselect**: `↑ / ↓` (all modes), `j / k` (when `navigationStyle == "vim"`), or `Ctrl-P / Ctrl-N` (when `navigationStyle != "vim"`) navigates rows in the active tab. Pressing `Esc` while a row is selected deselects it without closing the window.
- **Action Dispatch & Command Palette**: `Enter` toggles expansion (Memory) or opens URL (Bookmarks); `Delete / Backspace` deletes the selected item; `Cmd+K` opens the Command Palette pre-populated with available global & contextual actions (`Search`, `Toggle Sidebar`, `Switch to Memory/Bookmarks/Conversations`, `Edit Name`, `Edit URL`, `Delete`, `Deselect`, `Open`, `Refresh`, `Close Window`).
- **Window Management & Resizing**: `Cmd+W` or `Cmd+Q` closes the window via clean `orderOut(nil)`. Pressing `Esc` does not close the window. Window can be expanded freely without a hard max limit; shrinking causes row cards, labels, and the search field to wrap and collapse cleanly.
