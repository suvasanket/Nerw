...
| `NerwHubTabBarView.swift` | Vertical liquid glass pill straddling the left border of the panel, displaying tab icons separated by subtle dots, with the active tab rendered in bold white and inactive tabs in translucent white. |
...
| `Tabs/BaseHubListTab.swift` | Generic list controller base (`BaseHubListTab<Item>`) implementing `BaseHubTabProtocol`, bold header title (`22pt bold`), even panel padding (`24pt` top/bottom, `28pt` sides), `selectedIndex`, auto-scrolling, and row selection states. |
